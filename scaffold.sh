#!/usr/bin/env bash
# Netpay M0 scaffold. Run from the directory where you want `netpay/` to live.
...
mkdir -p \
  crates/netpay-core/src/{rules,formulas/province} \
  crates/netpay-core/tests/vectors \
  crates/netpay-wasm/src \
  crates/netpay-cli/src \
  packages/netpay-js \
  packages/netpay-py \
  packages/netpay-go \
...
[workspace]
resolver = "2"
members = ["crates/netpay-core", "crates/netpay-wasm", "crates/netpay-cli"]
...
cat > crates/netpay-core/Cargo.toml <<'EOF'
[package]
name = "netpay-core"
...
for c in netpay-wasm netpay-cli; do
...
[dependencies]
netpay-core = { path = "../netpay-core" }
EOF
done
echo 'pub fn placeholder() {}' > crates/netpay-wasm/src/lib.rs
printf 'fn main() {\n    println!("netpay cli placeholder");\n}\n' > crates/netpay-cli/src/main.rs

# ---------------------------------------------------------------- 4. core module skeleton
cat > crates/netpay-core/src/lib.rs <<'EOF'
...
: > crates/netpay-core/src/decimal.rs
: > crates/netpay-core/src/rounding.rs
