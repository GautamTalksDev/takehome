(function () {
  ...
  var P={AB:1,BC:1,MB:1,NB:1,NL:1,NS:1,NT:1,NU:1,ON:1,PE:1,QC:1,SK:1,YT:1,OutsideCanada:1};
  function province(r){r=String(r||'').trim();return P[r]?r:null}
  function root(s) {
    return new URL(s.src || s.getAttribute('src'), document.baseURI).origin + '/';
  }
  function mount(s) {
    ...
    var css = ':host{all:initial;display:block}div{font:1rem/1.4 system-ui,sans-serif;max-width:22rem;padding:.75rem 0;border:.0625rem solid #c8c8c8;border-width:.0625rem 0;color:#111}p{margin:.35rem 0}.n{font-size:1.5rem;font-weight:700;color:' + A + '}a{color:' + A + '}';
  ...
  function req(s) {
    var g = money(s.getAttribute('data-gross')), p = province(s.getAttribute('data-province'));
  ...
  import(root(pending[0]) + 'engine/takehome_wasm.js').then(function (m) {
